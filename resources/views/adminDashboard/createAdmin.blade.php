...
<div class="flex-grow h-screen p-4 bg-white rounded shadow-md overflow-y-scroll">
    ...
    <h3 class="font-bold text-2xl">Registar administrador</h3>

    <div id="errorDiv"></div>

    <form method="POST" action="{{ route('create-admin') }}" onsubmit="comparePassword(event)">
        @csrf
        <input type="email" name="email" class="mt-4 border-s border-gray-300 p-2 bg-gray-100 w-full"
            placeholder="E-mail" required>
        <br>
        <input type="password" name="password" class="mt-4 border-s border-gray-300 p-2 bg-gray-100 w-full"
            placeholder="Palavra-passe" id="passwordInput" required>
        <br>
        <input type="password" name="conf-password" class="mt-4 border-s border-gray-300 p-2 bg-gray-100 w-full"
