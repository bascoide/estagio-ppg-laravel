...
<div class="max-w-3xl w-full p-10 bg-white mt-10 shadow-lg mx-10 mb-10">
    ...
    @else
        ...
        <form method="POST" action="{{ route('user-upload-final-document') }}?final_document_id={{ $finalDocumentId }}"
            ...
            @csrf
            <input type="email" name="email" class="mt-4 border-s border-gray-300 p-2 bg-gray-200 w-full"
                placeholder="E-mail" required>
            <br>
            <input type="password" name="password" class="mt-4 border-s border-gray-300 p-2 bg-gray-200 w-full"
            ...
            <br>
            <br>
            <label for="document">Carregar documento final (.pdf):</label>
